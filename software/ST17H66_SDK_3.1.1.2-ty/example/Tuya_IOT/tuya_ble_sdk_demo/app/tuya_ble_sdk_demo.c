...
void tuya_ble_sdk_demo_init(void)
{
    ...
    tuya_ble_chx_key_pro_timer_init();
    key_driver_init(ty_ble_sdk_key_event_cb);
...
FN: 
*/
void tuya_ble_chx_key_pro_timer_cb(void)
{
    uint8_t timer_stop = 0;

    timer_stop |= chx_pro(CHX_KEY_PRO_TIMER_TIME_MS);

    timer_stop |= key_pro(CHX_KEY_PRO_TIMER_TIME_MS);
...
{
    tuya_ble_timer_create(&chx_key_pro_timer, CHX_KEY_PRO_TIMER_TIME_MS, TUYA_BLE_TIMER_SINGLE_SHOT, tuya_ble_chx_key_pro_timer_cb);
}
...
{
    tuya_ble_timer_start(chx_key_pro_timer);
}
